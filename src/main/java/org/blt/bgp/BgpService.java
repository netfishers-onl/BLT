...
import org.blt.bgp.config.nodes.impl.ClientConfigurationImpl;
import org.blt.bgp.config.nodes.impl.PeerConfigurationImpl;
...
import org.blt.bgp.net.attributes.bgplsnlri.BgpLsIPTopologyPrefixNLRI;
import org.blt.bgp.net.attributes.bgplsnlri.BgpLsLinkNLRI;
import org.blt.bgp.net.attributes.bgplsnlri.BgpLsNodeDescriptor;
import org.blt.bgp.net.attributes.bgplsnlri.BgpLsNodeNLRI;
import org.blt.bgp.net.attributes.bgplsnlri.IPPrefix;
...
import org.blt.snmp.SnmpPollingTask;
...
import org.blt.topology.net.Router.RouterIdentifier;
					...
					for (MultiProtocolNLRIInformation nlri : mpNlriAttribute.getNlris()) {
						if (nlri instanceof BgpLsNodeNLRI) {
							...
							try {
								...
								Link link = new Link(localId, remoteId, new Ipv4Subnet(
								    (Inet4Address) linkNlri.getLinkDescriptors()
								        .getIPv4InterfaceAddress(), 32), new Ipv4Subnet(
								    (Inet4Address) linkNlri.getLinkDescriptors()
							...
							try {
								RouterIdentifier routerId = new RouterIdentifier(
								    ...
								    node.getAreaId(), node.getBgpLsIdentifier());
								Router router = network.findOrAddRouter(routerId);
								for (IPPrefix ipPrefix : ipNlri.getPrefixDescriptor()
								    .getPrefixList()) {
